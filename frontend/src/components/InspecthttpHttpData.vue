<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            httpData # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            httpData
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <Date label="EndedDate" v-model="value.endedDate" :editMode="editMode" :inputUI="''"/>
            <String label="ResultData" v-model="value.resultData" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/httpData'), this.value)
                        } else {
